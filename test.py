        ...
        if isinstance(resstack, model.ResStack):
            for resblock in resstack.classifier:
                if isinstance(resblock, model.Resblock): 
                    ...
                    print(s(resblock.W))
                    print(torch.sum(torch.abs(fst_layer)) + torch.sum(torch.abs(snd_layer)))
                ...
                if isinstance(resblock, model.DownsamplingResBlock): 
                    ...
                    print(s(resblock.W))
                    print(torch.sum(torch.abs(fst_layer)) + torch.sum(torch.abs(snd_layer)) + torch.sum(torch.abs(last_layer)))
                    print()
                    sum += torch.sum(torch.abs(fst_layer))
                    sum += torch.sum(torch.abs(snd_layer))
...
#print_conv_layer_scaling_factors("/media/eddy/datasets/models/new/birdnet_final.pt")
